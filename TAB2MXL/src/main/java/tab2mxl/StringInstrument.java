		...
		for (int i = 2; str1.charAt(i) != '|'; i++) {
			...
			for (String j : allStrings) {
				...
				if (j != null && Character.isDigit(j.charAt(i))) {
					...
					output += "\t\t</notations>\n";
					output += "\t</note>\n";
					for (int s = 1; s < str1.length(); s++) {
						if ((s + 1) != str1.length() && str1.charAt(s) == '|' && str1.charAt(s + 1) == '-') {
							measureCount++;
							...
							for (String j : allStrings) {
								...
								if (j != null && Character.isDigit(j.charAt(i))) {
									...
									if (str6 == null && str7 == null) {
										note = Notes.bassNotes("String" + String.valueOf(stringNum),
												Character.getNumericValue(fret));
										octave = Notes.bassOctave("String" + String.valueOf(stringNum),
												Character.getNumericValue(fret));
									} else {
										note = Notes.guitarNotes("String" + String.valueOf(stringNum),
												Character.getNumericValue(fret));
										octave = Notes.guitarOctave("String" + String.valueOf(stringNum),
												Character.getNumericValue(fret));
									}
									body.append("<note>\n");
									body.append(" <pitch>\n");
